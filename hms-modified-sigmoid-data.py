...
def lambdaFunction(energy):
    # return (1+np.exp(8-16*energy))**(-1) + 0.5 # shifted vertically sigmoid
    # return 2 * (1 + np.exp(8 - 16 * energy)) ** (-1)  # increased sigmoid
    return 1 * (1 + np.exp(3.823 - 4.134 * energy)) ** (-1)  # different parameter values
    # return (1 + np.exp(70 - 100 * energy)) ** (-1) # increased magnitude parameter values
    # return 2 * (1 + np.exp(9 - 9 * energy)) ** (-1) # increased energy
...
# nSteps = np.array([4]) # np.linspace(1, 4, 4)
    ...
    for bStep in range(len(bSteps)):
        ...
        for step in range(len(energySteps)):
            ...
            for s in icX1:
                for v in icX2:
                    X0 = np.array([s, v])
                    # z = scipy.optimize.broyden1(H, [s, v], f_tol=1e-14)
                    # sol = scipy.optimize.root(H, X0, method='lm', options={
                    #                           'ftol': 1e-14, 'xtol': 1e-10, 'maxiter': 1000})
                    # sol = scipy.optimize.root(H, X0, method='lm')
            ...
            # csvFileName = "data-files/unique-steady-states-shifted-sigmoid.csv" # shifted vertically sigmoid
            # csvFileName = "data-files/unique-steady-states-increased-sigmoid.csv" # increased sigmoid
            csvFileName = "data-files/unique-steady-states-different-sigmoid-parameters.csv"  # changed sigmoid parameters
            ...
            # # export to csv file without index col
            # csvFileName3 = "data-files/number-stable-ss-shifted-sigmoid.csv" # shifted vertically sigmoid
            # csvFileName3 = "data-files/number-stable-ss-increased-sigmoid.csv" # increased sigmoid
            csvFileName3 = "data-files/number-stable-ss-different-sigmoid-parameters.csv"  # changed sigmoid parameters
