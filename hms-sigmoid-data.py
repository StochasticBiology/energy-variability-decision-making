...
def lambdaFunction(energy):
    return (1+np.exp(8-16*energy))**(-1) # original
    # return (1 + np.exp(70 - 100 * energy)) ** (-1) # increased parameter values
    ...
    for aStep in range(len(aSteps)):
        ...
        for bStep in range(len(bSteps)):
            ...
            for step in range(len(energySteps)):
                ...
                csvFileName = "data-files/unique-steady-states-n%.0f.csv" % n
                ...
                # # export to csv file without index col
                csvFileName3 = "data-files/number-stable-ss-n%.0f.csv" % n
